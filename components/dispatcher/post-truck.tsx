...
import { Truck as TruckIcon, Plus, Calendar, MapPin, Edit, Trash2, RefreshCw, FileText } from 'lucide-react'
...
export function PostTruck() {
  ...
  const fetchMyTrucks = async () => {
    setIsLoading(true)
    try {
      console.log('Fetching dispatcher trucks...')
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">Manage available trucks for your carriers</p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" onClick={fetchMyTrucks} disabled={isLoading}>
            <RefreshCw className={`h-4 w-4 ${isLoading ? 'animate-spin' : ''}`} />
          </Button>
          <Button onClick={() => setShowForm(true)} className="flex items-center gap-2 bg-purple-600 hover:bg-purple-700">
          ...
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1">
              <LocationAutocomplete
                placeholder="Filter by city, state, carrier, or truck type..."
                value={searchTerm}
      ...
      {isLoading ? (
        ...
        <div className="grid gap-4">
          {filteredTrucks.map((truck) => (
            <Card key={truck.id} className="border-l-4 border-l-purple-500">
              <CardContent className="pt-6">
