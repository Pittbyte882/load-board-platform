...
import { MapPin, Calendar, Package, DollarSign, User, Building, RefreshCw, MessageSquare, CheckCircle } from "lucide-react"
...
export function CarrierNegotiations() {
  const { user } = useAuth()
  ...
  const fetchNegotiations = async () => {
    ...
    setIsLoading(true)
    try {
      // Fetch negotiations where this carrier is the broker (receiving counter-offers)
      const response = await fetch(`/api/loads/negotiate?carrierId=${user.id}`)
      if (response.ok) {
    ...
    try {
      ...
      if (response.ok) {
        showToastWithLogo({
          ...
          message: `Accepted ${negotiation.negotiator_name}'s offer of $${negotiation.counter_offer.toLocaleString()}`,
          type: 'success'
        })
        fetchNegotiations()
      } else {
  ...
  const handleAcceptFinalTerms = async (negotiation: Negotiation) => {
    const finalRate = negotiation.final_rate || negotiation.counter_offer
    
    if (!confirm(`Accept final terms and book this load for $${finalRate.toLocaleString()}?`)) return

    try {
      const response = await fetch('/api/loads/negotiate/accept-final-terms', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          negotiationId: negotiation.id,
      ...
      if (response.ok) {
        showToastWithLogo({
          title: "Load Booked!",
          message: `Load ${negotiation.load_id} has been booked at $${finalRate.toLocaleString()}. Check your Booked Loads.`,
          type: 'success'
        })
        fetchNegotiations()
      } else {
        showToastWithLogo({
          ...
          message: "Failed to book load. Please try again.",
          type: 'error'
        })
      }
    } catch (error) {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'broker_accepted': return 'bg-green-100 text-green-800'
      case 'accepted': return 'bg-green-100 text-green-800'
      case 'final_accepted': return 'bg-blue-100 text-blue-800'
  ...
  const getStatusText = (status: string) => {
    switch (status) {
      case 'broker_accepted': return 'Accepted - Awaiting Your Confirmation'
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">
            Manage rate negotiations ({negotiations.filter(n => n.status === 'pending' || n.status === 'broker_accepted').length} requiring action)
          </p>
        </div>
      ...
      {negotiations.length === 0 ? (
        <Card>
          <CardContent className="p-12 text-center">
            ...
            <p className="text-gray-600">
              Rate negotiations and counter-offers will appear here.
            </p>
          </CardContent>
        </Card>
      ) : (
                ...
                <div className="flex items-start justify-between">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                      ...
                      <div className="bg-green-50 p-3 rounded-lg">
                        <p className="text-sm font-medium text-green-800">
                          {negotiation.status === 'broker_accepted' ? 'Final Rate' : 'Their Offer'}
                        </p>
                  ...
                  <div className="flex flex-col gap-2 ml-4">
                    ...
                    {negotiation.status === 'broker_accepted' && (
                      <Button
                        onClick={() => handleAcceptFinalTerms(negotiation)}
                        className="bg-blue-600 hover:bg-blue-700"
                        size="sm"
                      >
                        <CheckCircle className="h-4 w-4 mr-2" />
                        Accept Final Terms & Book Load
                      </Button>
                    )}
                  </div>
                </div>
      ...
      {/* Counter Offer Dialog - Same as before */}
